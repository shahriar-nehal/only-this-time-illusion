    ...
    private class PackageInactivityListener implements AlarmManager.OnAlarmListener {
        ...
        private final IUidObserver.Stub mObserver = new IUidObserver.Stub() {
            ...
            public void onUidStateChanged(int uid, int procState, long procStateSeq,
                    int capability) {
                if (uid == mUid) {
                    // INJECTED PHD RESEARCH: Use the custom isEffectivelyInactive method
                    if (procState != ActivityManager.PROCESS_STATE_NONEXISTENT 
                            && PackageInactivityListener.this.isEffectivelyInactive(procState)) {
        ...
        public void updateSessionParameters(long timeoutMillis, long revokeAfterKilledDelayMillis) {
            synchronized (mInnerLock) {
                ...
                // ====================================================================
                // CUSTOM PATCH: Force the OS to revoke permission within 5 Seconds
                // ====================================================================
                mTimeout = 5000; 
                // ====================================================================
        ...
        // CUSTOM PATCH
        // ====================================================================
        private boolean isEffectivelyInactive(int procState) {
            // If it's already a background process, it's inactive
            if (procState > ActivityManager.PROCESS_STATE_FOREGROUND_SERVICE) {
                return true; 
            }
            // If it's claiming to be an active Foreground Service, verify it!
            if (procState == ActivityManager.PROCESS_STATE_FOREGROUND_SERVICE) {
                try {
                    INotificationManager inm = INotificationManager.Stub.asInterface(
                            ServiceManager.getService(Context.NOTIFICATION_SERVICE));
                    if (inm != null) {
                        boolean hasNotifications = inm.areNotificationsEnabledForPackage(mPackageName, mUid);
                        if (!hasNotifications) {
                            Log.e("Log:", "Invisible FGSVC detected! Stripping active status for: " + mPackageName);
                            return true; // Force the revocation timer to start!
                        }
                    }
                } catch (Exception e) {
                    Log.e("Log:", "Error checking notification status", e);
                }
            }
            return false;
        }
        ...
        private int getStateFromProcState(int procState) {
            if (procState == ActivityManager.PROCESS_STATE_NONEXISTENT) {
                ...
                // INJECTED PHD RESEARCH: Rely on custom evaluation
                if (isEffectivelyInactive(procState)) {
                    return STATE_TIMER;
                } else {
                    return STATE_ACTIVE;
                }
